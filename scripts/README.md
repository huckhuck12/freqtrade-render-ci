# 脚本使用说明

本目录包含了用于测试和回测8PM高低点策略的各种脚本。

## 📁 目录结构

```
scripts/
├── local/                          # 本地运行脚本
│   ├── test_strategy.py            # 独立策略测试（推荐）
│   ├── run_local_backtest.sh       # 本地快速回测
│   └── run_freqtrade_backtest.sh   # 完整Freqtrade回测
├── ci/                             # CI/CD脚本
│   ├── prepare_backtest.sh         # GitHub Actions准备脚本
│   └── analyze_results.sh          # 结果分析脚本
└── README.md                       # 本文件
```

## 🚀 本地脚本使用

### 1. 独立策略测试 (推荐新手)

**特点**: 不需要freqtrade环境，使用模拟数据，快速验证策略逻辑

```bash
# 安装依赖
pip install pandas numpy matplotlib

# 运行测试
python scripts/local/test_strategy.py
```

**优势**:
- ✅ 无需复杂环境配置
- ✅ 快速验证策略逻辑
- ✅ 可自由修改参数
- ✅ 详细的交易记录

### 2. 本地快速回测

**特点**: 需要freqtrade环境，使用真实数据，快速回测

```bash
# 确保已安装freqtrade
pip install freqtrade

# 使用默认参数
./scripts/local/run_local_backtest.sh

# 自定义参数
./scripts/local/run_local_backtest.sh [时间范围] [交易对] [数据天数]
```

**参数说明**:
- `时间范围`: 格式为YYYYMMDD-YYYYMMDD，如20240101-20241231
- `交易对`: 空格分隔的交易对，如"ETH/USDT BTC/USDT"
- `数据天数`: 下载的历史数据天数，如180

**示例**:
```bash
# 回测2024年全年，使用ETH和BTC，下载365天数据
./scripts/local/run_local_backtest.sh 20240101-20241231 "ETH/USDT BTC/USDT" 365

# 回测最近6个月，仅使用ETH
./scripts/local/run_local_backtest.sh 20240701-20241231 "ETH/USDT" 180
```

### 3. 完整Freqtrade回测

**特点**: 完整的freqtrade回测流程，包含详细配置

```bash
./scripts/local/run_freqtrade_backtest.sh
```

## 🤖 CI/CD脚本

这些脚本用于GitHub Actions自动化回测，通常不需要手动运行。

### prepare_backtest.sh
- 检查CI环境
- 验证配置文件
- 创建必要目录

### analyze_results.sh
- 解析回测结果
- 提取关键指标
- 生成性能评估

## 📊 结果解读

### 关键指标说明

| 指标 | 说明 | 理想值 |
|------|------|--------|
| 总收益率 | 整个回测期间的总收益 | > 0% |
| 胜率 | 盈利交易占总交易的比例 | > 50% |
| 盈利因子 | 总盈利/总亏损 | > 1.5 |
| 最大回撤 | 最大的资金回撤幅度 | < 20% |
| 夏普比率 | 风险调整后收益 | > 1.0 |

### 性能评估标准

**优秀表现**:
- 胜率 ≥ 55%
- 盈利因子 ≥ 1.5
- 最大回撤 ≤ 15%

**一般表现**:
- 胜率 45-55%
- 盈利因子 1.0-1.5
- 最大回撤 15-25%

**需要优化**:
- 胜率 < 45%
- 盈利因子 < 1.0
- 最大回撤 > 25%

## 🔧 故障排除

### 常见问题

1. **freqtrade未安装**
   ```bash
   pip install freqtrade
   ```

2. **权限问题**
   ```bash
   chmod +x scripts/local/*.sh
   ```

3. **数据下载失败**
   - 检查网络连接
   - 确认交易对名称正确
   - 尝试减少数据天数

4. **jq工具未安装**
   ```bash
   # Ubuntu/Debian
   sudo apt-get install jq
   
   # macOS
   brew install jq
   
   # Windows (使用WSL或Git Bash)
   # 下载jq.exe并添加到PATH
   ```

### 调试技巧

1. **查看详细日志**
   ```bash
   # 在freqtrade命令后添加 --verbosity 3
   freqtrade backtesting --config config/eightpm_backtest.json --verbosity 3
   ```

2. **检查策略语法**
   ```bash
   freqtrade test-pairlist --config config/eightpm_backtest.json
   ```

3. **验证数据**
   ```bash
   freqtrade list-data --config config/eightpm_backtest.json
   ```

## 💡 优化建议

### 参数调优流程

1. **使用独立测试快速验证**
   ```bash
   python scripts/local/test_strategy.py
   ```

2. **修改参数并重新测试**
   - 编辑 `final_optimized_strategy.py`
   - 调整止损止盈比例
   - 修改过滤条件

3. **freqtrade环境验证**
   ```bash
   ./scripts/local/run_local_backtest.sh
   ```

4. **多市场验证**
   ```bash
   # 测试不同交易对
   ./scripts/local/run_local_backtest.sh 20240101-20241231 "BTC/USDT" 365
   ./scripts/local/run_local_backtest.sh 20240101-20241231 "SOL/USDT" 365
   ```

### 策略改进方向

1. **时间过滤**: 添加特定时间段过滤
2. **市场状态**: 根据波动率调整参数
3. **多时间框架**: 结合更高时间框架确认
4. **动态参数**: 根据市场条件动态调整

---

如有问题，请查看主README或提交Issue。